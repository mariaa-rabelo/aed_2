...
std::set<std::vector<pair<Airport,string>>> FlightOption::flightsMaxAirlineNumber(const Graph<Airport> *g, std::set<std::vector<Airport>>& paths, int maxAirlines){
    std::set<std::vector<pair<Airport,string>>> res;
    // numero de airlines
    int minSize = 100;
    ...
    for (const auto& path : paths) {
        ...
        std::vector<pair<Airport,string>> final_path;
        ...
        final_path.push_back(make_pair(verts_of_path[0]->getInfo(), ""));
            ...
            for (auto e: v->getAdj()) {
                ...
                if (!neig->isVisited()) {
                    if (pred[neig->getInfo()].size() == 1 && pred[neig->getInfo()][0].empty()){
                        ...
                        //pred[neig->getInfo()].insert(pred[neig->getInfo()].end(), pred[v->getInfo()].begin(),
                        //                             pred[v->getInfo()].end());
                    }
                    /*
                    for ( auto &path : pred[neig->getInfo()]){
                        //cout << path.top().first.getCode()<< endl;
                        pair<Airport, std::string> new_pair(v->getInfo(),e.getAirline()->getCode());
                        path.push(new_pair);
                        cout << "IHIHIH" <<path.top().first.getCode()<< endl;
                    }
                    */
                    aux.push(neig);
                    if (neig == dest) {
